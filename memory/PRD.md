...
## Backlog / Next
- 2026-06: **Webhook-first CRM integration.** On `POST /api/booking`, after slot validation, backend fires `LEAD_WEBHOOK_URL` (LeadConnector/GHL) with nicely formatted fields (first/last/full name, email, phone, company, industry, notes, human date/time, timezone, ISO start, appointment_when, source, submitted_at) BEFORE the Google Calendar step — so the lead reaches the CRM even if Calendar is down. Best-effort (never blocks booking on webhook failure). Debug endpoint `GET /api/booking/last-webhook` gated by `WEBHOOK_DEBUG` (preview only; exposes PII, keep OFF in prod). Verified 100% by testing_agent (iteration_4); real test POST to owner's LeadConnector webhook returned 200. Owner must set `LEAD_WEBHOOK_URL` on Render (added to render.yaml as sync:false) and re-push to GitHub. Preview uses httpbin echo for safe testing.
- 2026-06: Fixed Render build failure — removed `emergentintegrations` + private `litellm` wheel URL (both unused, Emergent-internal) from `requirements.txt`; replaced container-patched pins with a clean all-public minimal set (fastapi 0.110.1, starlette 0.37.2, uvicorn 0.25.0, pydantic 2.6.4, python-dotenv 1.0.1, email-validator 2.1.1, requests 2.32.3, tzdata 2024.1). Verified in a fresh venv (install + import + uvicorn serve) and on the live backend. Owner must re-push to GitHub before redeploying on Render.
